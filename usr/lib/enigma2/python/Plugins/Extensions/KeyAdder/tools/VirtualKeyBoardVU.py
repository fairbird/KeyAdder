...
def VirtualKeyBoardEntryComponent(keys, selectedKey,shiftMode=False):
	...
	for key in keys:
		...
		if key == "EXIT":
			...
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_esc))
		elif key == "BACKSPACE":
			width = key_backspace.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_backspace))
		elif key == "CLEAR":
			width = key_clr.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_clr))
		elif key == "SHIFT":
			width = shiftkey_png.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=shiftkey_png))
		elif key == "SPACE":
			width = key_space.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_space))
		elif key == "OK":
			width = key_ok.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_ok))
		#elif key == "<-":
		...
		#	res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(45, 45), png=key_right))
			...
			res.extend((
				MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_bg),
				MultiContentEntryText(pos=(x, 0), size=(width, high), font=0, text=key.encode("utf-8"), flags=RT_HALIGN_CENTER | RT_VALIGN_CENTER)
			))
		
		if selectedKey == count:
			width = key_sel.size().width()
			res.append(MultiContentEntryPixmapAlphaTest(pos=(x, 0), size=(width, high), png=key_sel))
...
class VirtualKeyBoardKeyAdder(Screen):
