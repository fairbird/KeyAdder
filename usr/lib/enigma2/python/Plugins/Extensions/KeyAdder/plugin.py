...
def Plugins(**kwargs):
    ...
    # Extensions menu and RED button in channel list added by ostend
    if DreamOS():
        Descriptors.append(
            PluginDescriptor(
                name=NAME,
        ...
        # Channel List Context Menu added by ostende
        Descriptors.append(
            PluginDescriptor(
                name="Manually add Key to current service [KeyAdder]",
                description="Manually add Key to current service [KeyAdder]",
                where=[PluginDescriptor.WHERE_CHANNEL_CONTEXT_MENU, PluginDescriptor.WHERE_CHANNEL_SELECTION_RED],
                fnc=keymenu
            )
        )
