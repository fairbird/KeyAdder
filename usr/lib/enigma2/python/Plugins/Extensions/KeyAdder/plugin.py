...
from enigma import eConsoleAppContainer, eDVBDB, iServiceInformation, eTimer, loadPNG, getDesktop, RT_WRAP, RT_HALIGN_LEFT, RT_VALIGN_CENTER, eListboxPythonMultiContent, gFont, eServiceReference
...
config.plugins.KeyAdder.showplugin = ConfigSelection(default = "All", choices = [
    ("All", _("Channellist context menu and Extensions menu")),
    ("Channellist", _("Channellist context menu")),
    ("Extensions", _("Extensions menu")),
    ])
...
class keyAdder_setup(ConfigListScreen, Screen):
	if reswidth == 2560:
		if DreamOS():
			skin = """
				<screen name="keyAdder_setup" position="center,center" size="840,708" title="keyAdder setup">
					...
					<widget source="global.CurrentTime" render="Label" position="5,5" size="826,50" font="Regular;35" halign="center" foregroundColor="#00ffa500" backgroundColor="#16000000" transparent="1">
		...
		skin = """
			<screen name="keyAdder_setup" position="center,center" size="620,398" title="keyAdder setup">
				...
				<widget source="global.CurrentTime" render="Label" position="5,5" size="371,30" font="Regular;25" halign="center" foregroundColor="#00ffa500" backgroundColor="#16000000" transparent="1">
					<convert type="ClockToText">Format:%d-%m-%Y	%H:%M:%S</convert>
				</widget>
	...
	def createSetup(self):
		...
		self.list.append(self.ShowPlugin)
...
def Plugins(**kwargs):
    ...
    # Show according to config
    show = config.plugins.KeyAdder.showplugin.value

    if show in ("All", "Extensions"):
        Descriptors.append(
            PluginDescriptor(
                name=NAME,
                description=DES,
    ...
    if show in ("All", "Channellist"):
        Descriptors.append(
            PluginDescriptor(
                name="Manually add Key to current service [KeyAdder]",
                description="Manually add Key to current service [KeyAdder]",
                where=PluginDescriptor.WHERE_CHANNEL_CONTEXT_MENU,
                fnc=keymenu
            )
        )

        if DreamOS():  # Optional: check DreamOS only for red button
            Descriptors.append(
                PluginDescriptor(
                    name="Manually add Key to current service [KeyAdder]",
                    description="Manually add Key to current service [KeyAdder]",
